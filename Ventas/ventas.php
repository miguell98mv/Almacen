...
echo "<h2 class='vendedor' style='color:black;'>Selecciona un vendedor</h2>";
}else{
echo "<h2 class='vendedor' style='color:black;'>Ventas de ".ucwords(mb_strtolower($_POST["nombre"]))."</h2>";
}
...
<form id='form1' action='' method='post'>
 ...
 <h2 onclick='mostrarTodo1(this)'>Willian</h2></a>
<input type='hidden' name='nombre' value='willian'>
 </form>";
echo "</div>";
echo "
<div class='container2'>
 ...
 <a style='text-decoration:none; color:white;'>
 <h2 onclick='mostrarTodo2(this)'>Edixon</h2></a>
<input type='hidden' name='nombre' value='edixon'>
 </form>";
echo "</div>";echo "</div>";
...
$array_nombre = $calendario->ventas1($vendedor);
...
$resultado = $calendario->fecha1($_POST["mostrar".$mes.""],$vendedor);
...
echo "</div>";
  ...
  foreach ($array_nombre as $producto) {
    ...
    if(isset($_POST["mostrar".$mes.""])){
    $resultado = $calendario->fecha($_POST["mostrar".$mes.""]);
      $dia_p =substr($resultado[0]["Fecha"],0,7);
        require "mes1.php";
        foreach ($resultado as $valor){
        $dia =substr($valor["Fecha"],0,7);
